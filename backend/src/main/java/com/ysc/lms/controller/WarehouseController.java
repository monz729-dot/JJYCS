...
import com.ysc.lms.dto.warehouse.*;
import com.ysc.lms.service.WarehouseService;
import com.ysc.lms.util.ApiResponse;
...
public class WarehouseController {
    ...
    public ResponseEntity<ApiResponse<ScanResponse>> processScan(
        ...
        log.info("Processing scan - labelCode: {}, scanType: {}, by user: {}", 
                request.getLabelCode(), request.getScanType(), userPrincipal.getUserId());
        
        try {
            ScanResponse response = warehouseService.processScan(request, userPrincipal.getUserId());
    ...
    public ResponseEntity<ApiResponse<BatchProcessResponse>> batchProcess(
        ...
        log.info("Processing batch operation - action: {}, labelCodes: {}, by user: {}", 
                request.getAction(), request.getLabelCodes().size(), userPrincipal.getUserId());
        
        try {
            BatchProcessResponse response = warehouseService.batchProcess(request, userPrincipal.getUserId());
    ...
    public ResponseEntity<ApiResponse<PagedResponse<InventoryItem>>> getInventory(
        ...
        try {
            // 창고 접근 권한 확인
            if (!warehouseService.hasWarehouseAccess(warehouseId, userPrincipal.getUserId(), userPrincipal.getRole())) {
    ...
    public ResponseEntity<ApiResponse<PagedResponse<ScanEvent>>> getScanEvents(
        ...
        try {
            ...
            ScanEventSearchFilter filter = ScanEventSearchFilter.builder()
                    ...
                    .startDate(startDate)
                    .endDate(endDate)
                    .userId(userPrincipal.getRole().equals("ADMIN") ? null : userPrincipal.getUserId())
                    .build();
    ...
    public ResponseEntity<ApiResponse<LabelGenerationResponse>> generateLabel(
        ...
        try {
            LabelGenerationResponse response = warehouseService.generateLabel(request, userPrincipal.getUserId());
    ...
    public ResponseEntity<byte[]> printLabels(
        ...
        try {
            byte[] pdfData = warehouseService.generateLabelsPdf(request, userPrincipal.getUserId());
    ...
    public ResponseEntity<ApiResponse<RepackPhotoResponse>> uploadRepackPhoto(
        ...
        try {
            RepackPhotoResponse response = warehouseService.uploadRepackPhoto(request, userPrincipal.getUserId());
